...
def _interactive_select_suite_and_task(args: Args) -> Args:
    ...
    print(f"\nFinal configuration: Suite={suite}, Task={task_id}")
    print("Note: Replan steps will be selected interactively during evaluation.")
...
def eval_libero(args: Args) -> None:
    ...
    consecutive_resample_count = 0  # Track consecutive 0 inputs
    prediction_counter = 0  # Track number of predictions for unique filenames

    # Setup directories
    prediction_video_dir = (
        pathlib.Path(args.video_out_path)
        / args.task_suite
        / f"task_{args.task_id}"
        / "predicted_videos"
    )
    prediction_video_dir.mkdir(parents=True, exist_ok=True)

    for episode_idx in tqdm.tqdm(range(num_trials),
                                 desc=f"Task{args.task_id}",
                                 leave=False):
        ...
        while t < max_steps + args.num_steps_wait:
            try:
                ...
                if not action_plan:
                    # Interactive replan_steps selection
                    print(f"\n{'='*70}")
                    print(f"Episode {episode_idx+1}/{num_trials} - Step {t}")
                    print(f"\n📹 Video Locations:")
                    print(f"  • Predicted future (model): ./output_videos/")
                    print(f"    (action_view1.mp4 & action_view2.mp4 - updated each prediction)")
                    print(f"  • Execution replay (robot): {args.video_out_path}/{args.task_suite}/task_{args.task_id}/")
                    print(f"    (episode_XXX_success/failure.mp4 - saved after each episode)")
                    print(f"\n💡 Predicted = model's imagination | Replay = actual execution\n")
                    print("Enter replan steps:")
                    print("  1-16: Execute this many steps from action chunk")
                    print("  0: Use resampling (consecutive 0s increase temperature)")
                    temperature_hint = f" (temp={1.0 * (consecutive_resample_count + 1):.2f})" if consecutive_resample_count >= 0 else ""
                    print(f"  Current resample level: {consecutive_resample_count}{temperature_hint}")
                    print(f"{'='*70}")
                    
                    replan_steps = -1
                    while replan_steps < 0:
                        replan_str = _safe_input("Select replan steps (1-16, or 0 for resample): ", "10")
                        try:
                            replan_steps = int(replan_str)
                            if replan_steps > 16:
                                print(f"Invalid input: {replan_steps} > 16. Please enter 1-16 or 0.")
                                replan_steps = -1
                            elif replan_steps < 0:
                                print("Invalid input: negative number. Please enter 1-16 or 0.")
                                replan_steps = -1
                        except ValueError:
                            print(f"Invalid input: '{replan_str}'. Please enter a number (1-16 or 0).")
                            replan_steps = -1
                    
                    # Handle resampling vs normal action
                    if replan_steps == 0:
                        consecutive_resample_count += 1
                        resample_level = consecutive_resample_count
                        logging.info(f"Using resampling with level {resample_level} (temperature={1.0*resample_level:.2f})")
                        action_result = eval_policy.get_action(resample=resample_level)
                    else:
                        consecutive_resample_count = 0  # Reset on non-zero input
                        logging.info(f"Using replan_steps={replan_steps}")
                        action_result = eval_policy.get_action()
                    
                    # Save predicted videos (HIL mode generates these)
                    prediction_counter += 1
                    video1_imgs = action_result.get('video1_images', [])
                    video2_imgs = action_result.get('video2_images', [])
                    
                    # Notify user - policy.py already saved videos to ./output_videos/
                    if video1_imgs and video2_imgs:
                        print(f"\n✅ Predicted videos updated: ./output_videos/action_view1.mp4 & action_view2.mp4")
                        print(f"   (Check these to see model's predicted future trajectory)\n")
                    else:
                        logging.warning("No predicted videos generated (HIL mode should generate these)")
                    
                    action_chunk = action_result['chunk actions'][0]
                    
                    if replan_steps > 0:  # Only use replan_steps if not in resample mode
                        ...
                        # In resample mode (replan_steps=0), use all predicted steps
                        action_plan.extend(action_chunk)
    ...
    logging.info(f"Total predictions made: {prediction_counter}")
    logging.info(f"Predicted videos: ./output_videos/ (action_view1.mp4, action_view2.mp4)")
    logging.info(f"Execution replays: {pathlib.Path(args.video_out_path) / args.task_suite / f'task_{args.task_id}'}")
    ...
    with open(results_file, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("LIBERO HIL Evaluation Results\n")
        ...
        f.write(f"Mode: Interactive replan steps selection\n")
        f.write(f"Total Predictions: {prediction_counter}\n")
        f.write(f"Predicted Videos: ./output_videos/ (action_view1.mp4, action_view2.mp4)\n")
        f.write("=" * 80 + "\n\n")
