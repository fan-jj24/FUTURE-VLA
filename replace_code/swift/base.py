...
class Template(ProcessorMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # modify to initialize Act/Rec id ranges
        self._init_actrec_id_ranges()
    ...
    def _init_actrec_id_ranges(self):
        """
        Initialize and cache Act/Rec token ID ranges using normal token IDs (before BOS).
        
        Layout:
        - Act: 2048 tokens, placed closest to BOS at the tail
        - Rec: 4096 tokens, placed immediately before Act range
        """

        tok = self.tokenizer
        # Find the boundary token ID (endoftext/eos/bos)
        boundary_id = tok.convert_tokens_to_ids("<|endoftext|>")
        if boundary_id is None:
            boundary_id = getattr(tok, "eos_token_id", None) or getattr(tok, "bos_token_id", None)

        assert isinstance(boundary_id, int), f"Cannot find boundary id for <|endoftext|>/eos/bos, got: {boundary_id}"
        ...
        B_START = "<|B_START|>"
        B_END   = "<|B_END|>"
        new_tokens = [A_START, A_END, B_START, B_END]
        n_added = tok.add_tokens(new_tokens, special_tokens=True)

        ACT_MAX = 2048
        REC_MAX = 4096

        # Allocate token ID ranges backwards from boundary
        act_base = boundary_id - ACT_MAX
        rec_base = act_base - REC_MAX                      # Rec: [rec_base .. act_base-1]

        assert rec_base >= 0, f"Not enough normal token ids before BOS to allocate: rec_base={rec_base}"

        # Cache the computed ranges and delimiters
        self.ACT_MAX = ACT_MAX
        self.REC_MAX = REC_MAX
        self.ACT_BASE_ID = act_base
        self.REC_BASE_ID = rec_base
        self._actrec_ranges_ready = True
        self.A_START_ID = tok.convert_tokens_to_ids("<|A_START|>")
        self.A_END_ID   = tok.convert_tokens_to_ids("<|A_END|>")
        self.B_START_ID = tok.convert_tokens_to_ids("<|B_START|>")
        self.B_END_ID   = tok.convert_tokens_to_ids("<|B_END|>")

        # Compile regex patterns for parsing Act/Rec tags
        self._TAG_SPLIT_RE = re.compile(r"(<Act>.*?</Act>|<Rec>.*?</Rec>)", re.S)
        self._ACT_RE = re.compile(r"<Act>(.*?)</Act>", re.S)
        self._REC_RE = re.compile(r"<Rec>(.*?)</Rec>", re.S)

        print(f"[Act] base={self.ACT_BASE_ID}, range=[{self.ACT_BASE_ID},{self.ACT_BASE_ID+ACT_MAX-1}]")
        print(f"[Rec] base={self.REC_BASE_ID}, range=[{self.REC_BASE_ID},{self.REC_BASE_ID+REC_MAX-1}]")
    ...
    def skip_stop_tokens(self, generate_ids: List[int], is_finished: bool = True) -> List[int]:
        ...
        # However, other stopAct_words will be printed.
        tokenizer = self.tokenizer
    ...
    def _codes_to_ids_A(self, codes: list[int]) -> list[int]:
        # Act: 2048
        ids = [self.A_START_ID]
        for a in codes:
            assert 0 <= a < self.ACT_MAX, f"Act code out of range: {a}"
            ids.append(self.ACT_BASE_ID + a)
        ids.append(self.A_END_ID)
        return ids

    def _codes_to_ids_B(self, codes: list[int]) -> list[int]:
        # Rec: 4096
        ids = [self.B_START_ID]
        for b in codes:
            assert 0 <= b < self.REC_MAX, f"Rec code out of range: {b}"
            ids.append(self.REC_BASE_ID + b)
        ids.append(self.B_END_ID)
        return ids

    def _split_ab_segments(self, text: str):
        TAG_SPLIT_RE = self._TAG_SPLIT_RE
        ACT_RE = self._ACT_RE
        REC_RE = self._REC_RE
        def _parse_int_list(payload: str) -> list[int]:
            ...
            for x in parts:
                ...
                try:
                    ...
                    raise ValueError(f"Bad int token {x!r} in payload: {payload!r}") from e
            return out

        if "<Act>" not in text and "<Rec>" not in text:
            return [text]
        ...
        for p in parts:
            if not p:
                continue
            p_strip = p.strip()
            m = ACT_RE.fullmatch(p_strip)
            if m:
                codes = _parse_int_list(m.group(1))
                segs.append(self._codes_to_ids_A(codes)) 
                continue

            m = REC_RE.fullmatch(p_strip)
            if m:
                codes = _parse_int_list(m.group(1))
                segs.append(self._codes_to_ids_B(codes))  
                continue
    ...
    def _simplify_context_list(self, context_list, loss_scale_list, inputs):
        """Merge and simplify context list with Act/Rec tag processing.
        
        This function processes the context list by:
        1. Parsing <Act> and <Rec> tags and converting them to token ID lists
        2. Merging consecutive string contexts with the same loss_scale
        3. Preserving non-string contexts (e.g., token lists) as-is
        
        Args:
            context_list: List of contexts (strings or token lists)
            loss_scale_list: Corresponding loss scale for each context
            inputs: Template inputs for preprocessing
            
        Returns:
            Tuple of (simplified_context_list, simplified_loss_scale_list)
        """
        # Preprocess: split special tokens and tokenize
        ...
        res = []
        res_loss_scale = []
        temp = []  # Buffer for consecutive strings with same loss_scale
        temp_loss_scale = 0.0

        def _flush_temp():
            """Flush buffered strings into result list"""
            nonlocal temp, temp_loss_scale
            if temp:
        ...
        for context, loss_scale in zip(context_list, loss_scale_list):
            # Parse Act/Rec tags in string contexts
            if isinstance(context, str):
                segments = self._split_ab_segments(context)  # Split by <Act>/<Rec> tags
            else:
                segments = [context]

            for seg in segments:
                if isinstance(seg, str) and (loss_scale == temp_loss_scale):
                    # Accumulate consecutive strings with same loss_scale
                    temp.append(seg)
                else:
                    # Flush buffer before handling different types or loss_scales
                    _flush_temp()
                    if isinstance(seg, str):
                        temp.append(seg)
                        temp_loss_scale = loss_scale
                    else:
                        # Non-string segment (e.g., Act/Rec token IDs)
                        res.append(seg)
                        res_loss_scale.append(loss_scale)
                        temp_loss_scale = loss_scale

        _flush_temp()  # Flush any remaining buffered strings
