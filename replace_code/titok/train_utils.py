...
try:
    from data import PretokenizedWebDataset
except ImportError:
    PretokenizedWebDataset = None
...
def eval_reconstruction(
    ...
    # Variables to save random samples for visualization
    import random
    saved_original_images = None
    saved_reconstructed_images = None
    saved_fnames = None
    
    # Randomly select a batch index to save samples from
    # Estimate total batches (use a reasonable default if not available)
    try:
        total_batches = len(eval_loader)
    except:
        total_batches = 10  # Default fallback
    
    target_batch_idx = random.randint(0, max(0, total_batches - 1))
    current_batch_idx = 0
    ...
    for batch in eval_loader:
        ...
        # Save random samples from the target batch for visualization
        if current_batch_idx == target_batch_idx and accelerator.is_main_process and saved_original_images is None:
            num_samples = min(2, original_images.shape[0])  # Save 2 images
            ...
            saved_original_images = torch.clone(original_images[selected_indices])
            saved_reconstructed_images = torch.clone(reconstructed_images[selected_indices])
            if "__key__" in batch:
                saved_fnames = [batch["__key__"][i] for i in selected_indices]
            else:
                saved_fnames = [f"eval_{i}" for i in selected_indices]
    ...
    # Generate visualization images if we have the necessary parameters
    if (saved_original_images is not None and output_dir is not None and 
        global_step is not None and config is not None and 
        logger is not None and accelerator.is_main_process):
        
        images_for_saving, images_for_logging = make_viz_from_samples(
            saved_original_images,
        ...
        if config.training.enable_wandb:
            accelerator.get_tracker("wandb").log_images(
                {f"Eval Reconstruction": images_for_saving},
                step=global_step
            )
        else:
            accelerator.get_tracker("tensorboard").log_images(
                {"Eval Reconstruction": images_for_logging}, step=global_step
            )
        ...
        for i, img in enumerate(images_for_saving):
            filename = f"{global_step:08}_s-{i:03}-{saved_fnames[i]}.png"
            path = os.path.join(root, filename)
            img.save(path)
        
        logger.info(f"Saved {len(images_for_saving)} evaluation reconstruction images to {root}")
