    ...
    def __init__(
        ...
        if dataset_with_class_label:
            ...
            train_processing_pipeline = [
                ...
                wds.map(filter_keys(set(["image", "filename"]))),
                wds.map_dict(
                    image=transform.train_transform,
                    handler=wds.warn_and_continue,
                ),
        ...
        # Expand URLs before passing to ResampledShards to avoid file not found warnings
        # Handle OmegaConf ListConfig types and file lists
        from omegaconf import ListConfig
        if isinstance(train_shards_path, (list, ListConfig)):
            expanded_train_shards = []
            for path in train_shards_path:
                expanded_train_shards.extend(wds.shardlists.expand_urls(path))
        elif isinstance(train_shards_path, str):
            # Check if it's a file list (txt file containing URLs)
            if train_shards_path.endswith('.txt'):
                with open(train_shards_path, 'r') as f:
                    expanded_train_shards = [line.strip() for line in f if line.strip()]
            else:
                expanded_train_shards = wds.shardlists.expand_urls(train_shards_path)
        else:
            expanded_train_shards = train_shards_path

        # Create train dataset and loader.
        pipeline = [
            wds.ResampledShards(expanded_train_shards),
        ...
        # Expand eval shards URLs if needed (handle OmegaConf.ListConfig and file lists)
        if isinstance(eval_shards_path, (list, ListConfig)):
            expanded_eval_shards = []
            for path in eval_shards_path:
                expanded_eval_shards.extend(wds.shardlists.expand_urls(path))
        elif isinstance(eval_shards_path, str):
            # Check if it's a file list (txt file containing URLs)
            if eval_shards_path.endswith('.txt'):
                with open(eval_shards_path, 'r') as f:
                    expanded_eval_shards = [line.strip() for line in f if line.strip()]
            else:
                expanded_eval_shards = wds.shardlists.expand_urls(eval_shards_path)
        else:
            expanded_eval_shards = eval_shards_path
        
        pipeline = [
            wds.SimpleShardList(expanded_eval_shards),
            wds.split_by_worker,
